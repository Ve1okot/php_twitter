...
function get_user_info($login) {
    return db_query("SELECT * FROM `posts` JOIN `users` WHERE `$login`;")->fetch();
}
...
function register_user($auth_data) {
    if (empty($auth_data) || !isset($auth_data['login']) || empty($auth_data['login']) || empty($auth_data['pass']) || empty($auth_data['pass2'])) return false;
...
function login($auth_data) {
    if (empty($auth_data) || !isset($auth_data['login'])) return false;
